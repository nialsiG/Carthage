...
func GetTutorial(level : int) -> String:
	if (level == 0):
		return "Bienvenue, cher colobe, dans l’ère Tortonien.\rPour avoir une chance d’évoluer dans le temps :\rNourris-toi de ces aliments pour survivre aux aléas de la nuit\rSors de cet environnement pour en trouver un plus propice\r"
